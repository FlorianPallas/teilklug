...
const emptyEntry = (): Entry => ({ id: nextId.value++, price: 0, userIds: [] })

const entries = ref<Entry[]>(loadEntries() ?? [emptyEntry()])
...
<template>
  <main class="flex flex-col h-full max-h-screen sm:max-w-md mx-auto">
    ...
    <!-- Entries -->
    <ul class="p-4 gap-2 flex grow flex-col overflow-y-auto" ref="containerElement">
      <li
        v-for="entry in entries"
        :key="entry.id"
        :class="
          'bg-gray-800 text-white py-2 px-4 rounded-lg flex justify-end items-center border transition-colors ' +
    ...
    <div class="bg-gray-800 p-4 flex flex-col gap-4 sm:rounded-t-lg">
      ...
      <ul class="flex gap-2 justify-center">
        <button
          ...
          :key="user.id"
          :class="
            'bg-gray-700 text-gray-200 py-1 px-4 rounded-full border transition-colors ' +
      ...
      <div class="flex items-center gap-2 justify-between">
        <button
          class="fill-gray-200 bg-gray-700 p-2 rounded-full flex gap-2 items-center text-sm disabled:opacity-50"
          @click="deleteEntry"
          @mousedown.prevent=""
